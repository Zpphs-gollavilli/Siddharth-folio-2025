...
export class AchievementsArea extends Area {
  ...
  setWaterfall() {
    ...
    // Drop foam
    {
      const colorNode = Fn(() => {
        ...
        {
          const uv3 = baseUv
            .sub(vec2(0, this.game.ticker.elapsedScaledUniform.mul(0.11)))
            .mul(vec2(0.7, 0.6));
          const noise3 = texture(this.game.noises.voronoi, uv3).r;
    ...
    // Particles
    {
      ...
      // If marker has children (very common)
      if (reference.children.length >= 2) {
        reference.children[0].getWorldPosition(origin);
        reference.children[1].getWorldPosition(destination);
      } else {
        // Fallback: use forward direction
        reference.getWorldPosition(origin);
        destination.copy(origin).add(
          new THREE.Vector3(0, -3, 0) // waterfall drop direction
        );
      }
      ...
      material.positionNode = Fn(() => {
        ...
        const finalPosition = newPositionGeometry
  ...
  setPillar() {
    ...
    // Glyphs
    {
      ...
      const emissiveDefaultMaterial = this.game.materials.getFromName(
        "emissiveOrangeRadialGradient"
      );
      const emissiveAchievedMaterial = this.game.materials.getFromName(
      ...
      material.outputNode = Fn(() => {
        ...
        const glyph = texture(
          this.game.resources.achievementsGlyphsTexture,
          glyphUv
        ).r;
        ...
        const emissiveOutput = mix(
          ...
          float(instanceIndex)
            .div(count)
            .step(this.game.achievements.globalProgress.ratioUniform)
        );
