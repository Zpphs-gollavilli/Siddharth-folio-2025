...
export class CookieArea extends Area {
  ...
  setParticles() {
    const emissiveMaterial = this.game.materials.getFromName(
    ...
    material.positionNode = Fn(() => {
      ...
      progress.assign(
        newPosition.y.add(this.localTime.mul(newPosition.y)).fract()
      );
  ...
  setCookies() {
    ...
    this.cookies = {};
    this.cookies.spawnerPosition =
      this.references.items.get("spawner")[0].position;
    ...
    baseCookie.material = baseCookie.material.clone();
    this.game.materials.updateObject(baseCookie);
  ...
  setCounter() {
    ...
    this.counter.init = () => {
      ...
      mesh.quaternion.copy(
        this.references.items.get("counterLabel")[0].quaternion
      );
    ...
    this.counter.update = () => {
      ...
      context.fillText(
        formatedValue,
        canvas.width / 2,
        canvas.height * 0.5 + textOffsetVertical
      );
  ...
  accept() {
    ...
    this.cookies.currentIndex =
      (this.cookies.currentIndex + 1) % this.cookies.count;

    this.cookies.visibleCount = Math.min(
      this.cookies.visibleCount + 1,
      this.cookies.count
    );

    // Oven heat
    this.ovenHeat.scale.z = 2;
    gsap.to(this.ovenHeat.scale, {
      z: 1,
      overwrite: true,
      duration: 2,
      delay: 0.2,
  ...
  update() {
    // Time
    const timeScale =
      (Math.sin(this.game.ticker.elapsedScaled) * 0.3 + 0.5) * 0.3;
    this.localTime.value += this.game.ticker.deltaScaled * timeScale;

    // Blower
    this.blower.scale.y =
      Math.sin(this.game.ticker.elapsedScaled + Math.PI) * 0.25 + 0.75;
    ...
    if (this.cookies.visibleCount) {
      ...
      for (const fan of this.cookies.objects)
        allCookiesSleeping =
          allCookiesSleeping && fan.physical.body.isSleeping();
