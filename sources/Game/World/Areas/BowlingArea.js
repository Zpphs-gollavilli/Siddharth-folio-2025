...
export class BowlingArea extends Area {
  ...
  setPins() {
    ...
    // References
    // --------------------------------------------------
    const pinPositionsGroup = this.references.items.get("pinPositions")?.[0];

    const basePin = this.references.items.get("pinPhysicalDynamic")?.[0];

    if (!pinPositionsGroup || !basePin) return;

    // --------------------------------------------------
    // Pin position references (empties)
    // --------------------------------------------------
    const references = InstancedGroup.getReferencesFromChildren(
      pinPositionsGroup.children
    );

    if (!references || references.length === 0) return;

    // --------------------------------------------------
    // Base pin (visual source for instancing)
    // --------------------------------------------------
    ...
    basePin.visible = false;

    // --------------------------------------------------
    // Extract colliders ONCE (important)
    // --------------------------------------------------
    const descriptions = this.game.objects.getFromModel(basePin, {}, {});

    const colliders = descriptions?.[1]?.colliders;

    if (!colliders) return;

    // --------------------------------------------------
    // Create physics pins (NO visuals here)
    // --------------------------------------------------
    for (let i = 0; i < references.length; i++) {
      ...
      const pin = {
        ...
        object: null,
        body: null,
        basePosition: reference.position.clone(),
        baseRotation: reference.quaternion.clone(),
      };
    ...
    // --------------------------------------------------
    // Instanced rendering (ONCE, visuals only)
    // --------------------------------------------------
    ...
    this.pins.reset = () => {
      for (const pin of this.pins.items) {
        ...
        this.game.ticker.wait(2, () => pin.body.sleep());
      }
  ...
  setBall() {
    ...
    if (!baseBall || !baseBall.userData?.object?.physical?.body) {
      this.ball = null;
      return;
    }
    ...
    this.ball.reset = () => {
      ...
      this.game.ticker.wait(2, () => this.ball.body.sleep());
    };
  }
  ...
  setScreen() {
    ...
    const offsetPosition = Fn(([threshold]) => {
      const active = step(
        texture(this.dataTexture, uv()).r.sub(threshold).abs(),
        0.1
      );
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(discsStrength, "value", {
        label: "discsStrength",
        min: 0,
        max: 10,
        step: 0.001,
      ...
      debugPanel.addBinding(crossesStrength, "value", {
        label: "crossesStrength",
        min: 0,
        max: 10,
        step: 0.001,
      });
  ...
  setJukebox() {
    ...
    const rotationNode = Fn(() => {
      return sin(
        this.game.ticker.elapsedScaledUniform
          .mul(4)
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(notesStrength, "value", {
        label: "notesStrength",
        min: 0,
        max: 10,
        step: 0.001,
  ...
  update() {
    if (!this.screen || !this.game.player || !this.ball) return;

    let showRestartInteractivePoint = false;

    // Screen position
    if (this.screen.min !== undefined && this.screen.max !== undefined) {
      const targetX = clamp(
      ...
      this.screen.x +=
        (targetX - this.screen.x) * this.game.ticker.deltaScaled * 2;
      ...
      // const floatY =
      //   Math.sin(this.game.ticker.elapsedScaled * 0.3) * 0.5;
    ...
    // Screen strike label
    if (this.won)
      this.screen.labelStrike.visible =
        (this.game.ticker.elapsedScaled - this.wonTime) % 3 < 1.5;
    ...
    if (pinStateChanged) {
      ...
      if (!this.won) {
        const allDown = this.pins.items.reduce(
          (accumulator, pin) => accumulator && pin.isDown,
          true
        );
        if (allDown) {
          this.won = true;
    ...
    if (!ballIsSleeping) {
      const ballPosition = new THREE.Vector3().copy(
        this.ball.body.translation()
      );
    ...
    if (
      showRestartInteractivePoint &&
      this.restartInteractivePoint.state === InteractivePoints.STATE_HIDDEN
    )
